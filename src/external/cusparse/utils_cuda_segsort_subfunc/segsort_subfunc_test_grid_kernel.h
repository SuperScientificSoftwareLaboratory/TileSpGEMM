...
template<class T>
void scan_num_blk(T *in, const int length)
{
...
template<class T>
__global__
void kern_block_sort(uintT *key, T *val, uintT *keyB, T *valB, int *segs, 
...
template<class T>
__global__
void kern_block_merge(uintT *keys, T *vals, uintT *keysB, T *valsB, int *segs, int *bin, 
        int *blk_innerid, int *blk_seg_start, int length, int n, int stride)
{
    __shared__ uintT smem[128*16];
    const int tid = threadIdx.x;
    const int bid = blockIdx.x;
    int innerbid = blk_innerid[bid];
    int bin_it  = blk_seg_start[bid];
...
template<class T>
__global__
void kern_copy(uintT *srck, T *srcv, uintT *dstk, T *dstv, int *segs, int *bin, 
...
template<class T>
int gen_grid_kern_r2049(uintT *keys_d, T *vals_d, uintT *keysB_d, T *valsB_d, 
        int n, int *segs_d, int *bin_d, int bin_size, int length)
{
    int *blk_stat_d; // histogram of how many blocks for each seg
    cudaMalloc((void **)&blk_stat_d, bin_size*sizeof(int));
    int blk_num; // total number of blocks
    int *max_segsize_d;
    cudaMalloc((void **)&max_segsize_d, sizeof(int));
    cudaMemset(max_segsize_d, 0, sizeof(int));
    dim3 blocks(256, 1, 1);
    dim3 grids((bin_size+blocks.x-1)/blocks.x, 1, 1);
    // this kernel gets how many blocks for each seg; set flags to -1 for each 
    // seg; get max seg length
    ...
    kern_get_num_blk_init<<<grids, blocks>>>(max_segsize_d, segs_d, bin_d, blk_stat_d, 
            n, bin_size, length, 2048); // 512t*4k /*** codegen ***/
    int max_segsize;
    cudaMemcpy(&max_segsize, max_segsize_d, sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(&blk_num, blk_stat_d+bin_size-1, sizeof(int), cudaMemcpyDeviceToHost);
    scan_num_blk(blk_stat_d, bin_size);
    int tmp;
    cudaMemcpy(&tmp, blk_stat_d+bin_size-1, sizeof(int), cudaMemcpyDeviceToHost);
    blk_num = blk_num + tmp;
    int *blk_innerid; // record each block's inner id
    cudaMalloc((void **)&blk_innerid, blk_num*sizeof(int));
    int *blk_seg_start; // record each block's segment's starting position
    cudaMalloc((void **)&blk_seg_start, blk_num*sizeof(int));
    ...
    cudaMemcpy(&tmp, blk_stat_d+bin_size-1, sizeof(int), cudaMemcpyDeviceToHost);
    blk_num = blk_num + tmp;
    cudaFree(blk_innerid); blk_innerid = NULL;
    cudaMalloc((void **)&blk_innerid, blk_num*sizeof(int));
    cudaFree(blk_seg_start); blk_seg_start = NULL;
    cudaMalloc((void **)&blk_seg_start, blk_num*sizeof(int));
    grids.x = (blk_num+blocks.x-1)/blocks.x;
    kern_get_init_pos<<<grids, blocks>>>(blk_stat_d, blk_innerid, blk_seg_start, 
            blk_num, bin_size);
    ...
    blocks.x = 128;
    grids.x = blk_num;
    uintT *srck = (cnt&1)?keys_d:keysB_d;
    uintT *dstk = (cnt&1)?keysB_d:keys_d;
    ...
    cudaFree(blk_stat_d);
    cudaFree(blk_innerid);
    cudaFree(blk_seg_start);
