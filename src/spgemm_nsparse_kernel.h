...
void init_bin(sfBIN *bin, int M)
{
    ...
    // cuda_memory_use[*index] = cuda_memory_use[(*index) - 1] + sizeof(int) * M;
    // (*index) += 1;

    cudaMalloc((void **)&(bin->d_row_nz), sizeof(int) * (M + 1));
    // gettimeofday(&tv, NULL );
    // time_node[*index] = tv.tv_sec * 1000.0 + tv.tv_usec / 1000.0;
    // cuda_memory_use[*index] = cuda_memory_use[(*index) - 1] + sizeof(int) * (M + 1);
    // (*index) += 1;
    ...
    // gettimeofday(&tv, NULL );
    // time_node[*index] = tv.tv_sec * 1000.0 + tv.tv_usec / 1000.0;
    // cuda_memory_use[*index] = cuda_memory_use[(*index) - 1] + sizeof(int) * BIN_NUM;
    // (*index) += 1;
                                    ...
                                    int *d_row_nz, int bin_offset, int M)
{
    ...
    for (j = d_arpt[rid]; j < d_arpt[rid + 1]; j += WARP) {
        ...
        for (l = 0; l < WARP && j + l < d_arpt[rid + 1]; l++) {
            ccol = __shfl_sync(0xffffffff, acol, l);//__syncwarp();
    ...
    for (j = WARP / 2; j >= 1; j /= 2) {
        nz += __shfl_xor_sync(0xffffffff,nz, j);//__syncwarp();
    }
...
//__syncwarp();
    ...
    for (j = tid; j < (B_PWMIN); j += PWARP) {
        shared_check[soffset + j] = -1;//__syncwarp();
//        shared_value[soffset + j] = 0;
    ...
    //__syncwarp();
    rid = d_row_perm[rid + bin_offset];//__syncwarp();
...
//        aval = ld_gbl_real(d_aval + j);
        for (k = d_brpt[acol]; k < d_brpt[acol + 1]; k++) {//__syncwarp();
            bcol = d_bcol[k];//__syncwarp();
//            bval = d_bval[k];
	
            key = bcol;//__syncwarp();
            hash = (bcol * HASH_SCAL) & ((B_PWMIN) - 1);//__syncwarp();
            adr = soffset + hash;//__syncwarp();
            while (1) {
                if (shared_check[adr] == key) {
//                    atomic_fadd(shared_value + adr, aval * bval);
                    break;
                }
                else if (shared_check[adr] == -1) {//__syncwarp();
                    old = atomicCAS(shared_check + adr, -1, key);//__syncwarp();
                    if (old == -1) {
//                        atomic_fadd(shared_value + adr, aval * bval);
                        break;
                    }
...
//            shared_value[soffset + index] = shared_value[soffset + j];
    ...
    int nz = d_nz[rid];//__syncwarp();
    // Sorting for shared data
    int count, target;//__syncwarp();
    for (j = tid; j < nz; j += PWARP) {
//__syncwarp();
        target = shared_check[soffset + j];//__syncwarp();
        count = 0;//__syncwarp();
        for (k = 0; k < nz; k++) {//__syncwarp();
...
//d_crow[offset + j] = rid;
//        d_ccol[offset + j] = shared_check[soffset + j];
    }
}
...
//d_crow[offset + j] = rid;
//        d_ccol[offset + j] = d_check[doffset + j];
    }
...
void spgemm_kernel_hash(sfCSR *a, sfCSR *b, sfCSR *c)
{
    ...
    cudaMalloc((void **)&(c->d_rpt), sizeof(int) * (M + 1));
    ...
    cudaMalloc((void **)&(c->d_col), sizeof(int) * c->nnz);
    cudaMalloc((void **)&(c->d_val), sizeof(real) * c->nnz);
                 ...
                 int *nnz)
{
    ...
    for (i = BIN_NUM - 1; i >= 0; i--) {
        if (bin->bin_size[i] > 0) {
            ...
            case 6 :
                    ...
                    // gettimeofday(&tv, NULL );
                    // time_node[*index] = tv.tv_sec * 1000.0 + tv.tv_usec / 1000.0;
                    // cuda_memory_use[*index] = cuda_memory_use[(*index) - 1] + sizeof(int) * bin->bin_size[i];
                    // (*index) += 1;
            	    ...
            	    if (fail_count > 0) {
                        ...
                        // gettimeofday(&tv, NULL );
                        // time_node[*index] = tv.tv_sec * 1000.0 + tv.tv_usec / 1000.0;
                        // cuda_memory_use[*index] = cuda_memory_use[(*index) - 1] + sizeof(int) * table_size;
                        // (*index) += 1;
    ...
    //cudaMemcpy(d_crpt, bin->d_row_nz, (M+1) * sizeof(int),   cudaMemcpyDeviceToDevice);

//exclusive_scan_device_cuda(d_crpt, M + 1);
//exclusive_scan_device_cuda_thrust<int>(d_crpt, M+1);
thrust::exclusive_scan(thrust::device, bin->d_row_nz, bin->d_row_nz + M+1, d_crpt, 0);
			     ...
			     int M, int N)
{
  ...
  for (i = BIN_NUM - 1; i >= 0; i--) {
	...
	case 6 :
	  {
	    int max_row_nz = N *2; //90000; //bin->max_nz * 2;
	    int table_size = max_row_nz * bin->bin_size[i];
//printf("nsparse max_row_nz = %i, bin->max_nz = %i, bin_size[i] = %i\n", max_row_nz, bin->max_nz, bin->bin_size[i]);
	    int *d_check;
	    real *d_value = NULL;
	    cudaMalloc((void **)&(d_check), sizeof(int) * table_size);
        // gettimeofday(&tv, NULL );
...
//	    cudaMalloc((void **)&(d_value), sizeof(real) * table_size);
	    BS = 1024;
	    GS = div_round_up(table_size, BS);
	    init_check<<<GS, BS, 0, bin->stream[i]>>>(d_check, table_size);
