...
int partition(int *key, int length, int pivot_index)
{
    int i  = 0 ;
    int small_length = pivot_index;

    int pivot = key[pivot_index];
    swap(&key[pivot_index], &key[pivot_index + (length - 1)]);

    for(; i < length; i++)
    {
        if(key[pivot_index+i] < pivot)
        {
            swap(&key[pivot_index+i],  &key[small_length]);
            small_length++;
        }
    }

    swap(&key[pivot_index + length - 1],  &key[small_length]);

    return small_length;
}

// quick sort key (child function)
int partition(double *key, int length, int pivot_index)
{
    int i  = 0 ;
    int small_length = pivot_index;

    double pivot = key[pivot_index];
    swap(&key[pivot_index], &key[pivot_index + (length - 1)]);
...
void quick_sort_key(double *key, int length)
{
    if(length == 0 || length == 1)
        return;

    int small_length = partition(key, length, 0) ;
